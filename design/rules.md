...
Only Human does not provide a setting or any of the rules associated with that, but this document also provides an implementation of the framework called [Still Human](#still-human), detailed later in this document. Still Human is very light on setting details, but it is fleshed out enough to be actually played as a game. You can of course add to it, or create other implementations of the framework.
...
The process of playing the game involves the GM describing a scenario the players' characters find themselves in, and then the players describing how their characters react. The GM then adjudicates the outcome of the character's actions, possibly calling on the players to take **tests** to determine the outcome, or a component of the outcome, of their actions.

The GM will also take actions for any NPCs or other entities, and in general run the rest of the world. If there are any rules disputes, or in the likely event that something comes up that the rules do not handle, you are encouraged to discuss it, but the GMs decision on the matter is final.
...
For example, a player have have allocated `1` point to their character's `DEX` **stat**, `2` points to their character's `INT` **stat**, `5` points to their character's _Ranged Attacking_ **skill**, and `3` points to their character's _Slight of Hand_ **skill**. Their character would then have a _Ranged Attacking_ **modifier** of `1 (DEX) + 2 (INT) + 5 (Ranged Attacking) = 8` and a _Ranged Attacking_ **DC** of `8 + 10 = 18`. Further, their character would have a _Slight of Hand_ **modifier** of `1 (DEX) + 1 (DEX) + 3 (Slight of Hand) = 4` and a _Slight of Hand_ **modifier** of `4 + 10 = 14`.
